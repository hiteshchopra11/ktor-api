...
fun Route.notesRoutes() {
    ...
    get("/notes") {
        when (val sortType = call.request.queryParameters["sort"]) {
            ...
            null -> {
                call.respond(HttpStatusCode.OK, service.fetchAllNotes())
            }
    ...
    get("/paginatedNotes") {
        ...
        call.respond(HttpStatusCode.OK, service.fetchAllPaginatedNotes(page, size))
    }
    ...
    get("/notes/{id}") {
        ...
        if (note == null) {
            call.respond(
                ...
                message = NoteResponse(success = false, data = "Could not find note with that id = $id")
            )
        } else {
            call.respond(
                HttpStatusCode.OK,
